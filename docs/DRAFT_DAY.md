...
| `Confidence`      | no       | `HIGH` / `MEDIUM` / `LOW` only                                           | Anything else (including `REVIEW`) rejects the row. Review status is workflow, not confidence: a row needing human verification uses `LOW` + `Review Fields` + `QA Note`. |
| `Review Fields`   | no       | `;`-separated, e.g. `FGM/A;PTS` (a trailing `*` as in `GP*` is accepted) | A flagged **stat** rejects the row (never inferred). Flagged `team`/`pos`/`rank` are nulled with a warning.                                                               |
...
- **Data → Reconciliation** shows `matched · market-only · projection-only · ambiguous · needs review · team mismatch`.
...
- A projected player **without a Yahoo market record** keeps its value and rank but shows **NO MARKET**: market urgency is unavailable, so the app never says DRAFT NOW / WAIT / SAFE WAIT for them.
- Category labels are sample-size aware: after 0–2 of your picks they show direction only (leaning + / even / leaning −); after 3–4 a weakness may be shown as WEAK; CRITICAL appears from pick 5. Hover a row for the calculated state and numbers.
