...
const ctx = (over: Partial<PlayerContext> = {}): PlayerContext => ({ canonicalPlayerId: 'p', age: 26, currentStatus: 'HEALTHY', ...over });

describe('availability score', () => {
  it('recurrence class matters: same games missed, HIGH ≫ LOW', () => {
    const low = computeAvailability([season('2025', 62, [{ games: 20, recurrence: 'LOW' }])], ctx(), null, cfg);
    const high = computeAvailability([season('2025', 62, [{ games: 20, recurrence: 'HIGH' }])], ctx(), null, cfg);
  ...
  it('three-season weighting 50/30/20 (renormalized when fewer seasons)', () => {
    const h = computeAvailability(
      [season('2025', 82), season('2024', 82 - 41), season('2023', 82)].map((s) => (s.gamesPlayed < 82 ? { ...s, absences: [{ games: 41, recurrence: 'HIGH' as const }] } : s)),
      ctx(),
      null,
      cfg,
    );
    expect(h.terms.history).toBeCloseTo(0.3 * 0.5);
    const two = computeAvailability([season('2025', 82), season('2024', 41, [{ games: 41, recurrence: 'HIGH' }])], ctx(), null, cfg);
  ...
  it('chronic pattern, age, current status and manual note add risk', () => {
    const hist = [season('2025', 70, [{ games: 12, recurrence: 'HIGH' }]), season('2024', 70, [{ games: 12, recurrence: 'HIGH' }])];
    const a = computeAvailability(hist, ctx({ age: 34, currentStatus: 'OUT_LONG', manualRiskDelta: 0.1 }), null, cfg);
  ...
  it('risk bands', () => {
    ...
    expect(computeAvailability([], ctx({ currentStatus: 'OUT_LONG', manualRiskDelta: 0.3 }), null, cfg).risk).toBe('VERY_HIGH');
  });
});
...
describe('round-dependent risk (R3-1)', () => {
  ...
  it('residual weight applies only to historical risk; current status is applied in full', () => {
    ...
    const r0 = computeAvailability(hist, ctx({ currentStatus: 'OUT_LONG' }), null, { ...cfg, durabilityResidualWeight: 0 });
    const r5 = computeAvailability(hist, ctx({ currentStatus: 'OUT_LONG' }), null, { ...cfg, durabilityResidualWeight: 0.5 });
    const r1 = computeAvailability(hist, ctx({ currentStatus: 'OUT_LONG' }), null, { ...cfg, durabilityResidualWeight: 1 });
