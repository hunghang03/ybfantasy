...
function atPick11(ds: Dataset): DraftEvent[] {
  const order = ds.market
    .filter((m) => m.canonicalPlayerId.startsWith('S'))
...
describe('pick-pair ordering (deterministic, ordinal)', () => {
  it('take the at-risk star now when the slightly better star will safely last', () => {
    ...
    const ds = withSynth(synthDataset(pool), [
      star('AT_RISK', 1.0, 12),
      star('SAFE_BETTER', 1.02, 60),
      star('SAFE_OTHER', 0.9, 70),
    ]);
  ...
  it('the recommended player is always LEAN-DRAFT quality on its own when such players exist', async () => {
    ...
    for (let n = 0; n < 120; n++) {
      ...
      const next = order.find(
        (m) => !e.some((x) => x.type === 'PICK' && x.playerId === m.canonicalPlayerId),
      )!;
      const r = appendPick(
        e,
        {
          playerId: ev.timing.onTheClock ? ev.recommendedId! : next.canonicalPlayerId,
          by: ev.timing.onTheClock ? 'ME' : 'OTHER',
          at: 't',
        },
        14,
        13,
