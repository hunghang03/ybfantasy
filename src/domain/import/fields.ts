...
const NAME: FieldSpec = { key: 'name', label: 'Player name', required: true, synonyms: ['player', 'name', 'player name', 'playername', 'full name'] };
const TEAM: FieldSpec = { key: 'team', label: 'NBA team', required: false, synonyms: ['team', 'tm', 'nba team', 'teamabbr'] };
const POS: FieldSpec = { key: 'positions', label: 'Positions', required: false, synonyms: ['pos', 'position', 'positions', 'eligible', 'elig'] };
const PID: FieldSpec = { key: 'providerPlayerId', label: 'Provider player ID', required: false, synonyms: ['id', 'player id', 'playerid', 'provider id', 'yahoo id', 'yahoo_id'] };
...
export const FIELD_SPECS: Record<ImportKind, FieldSpec[]> = {
  PROJECTION: [
    ...
    POS,
    PID,
    { key: 'gp', label: 'Games played', required: true, synonyms: ['gp', 'g', 'games', 'proj gp', 'projected gp'] },
    ...
    { key: 'fga', label: 'FGA', required: false, synonyms: ['fga', 'fg att', 'fg attempts'], help: 'Required unless FG% cell contains (makes/attempts)' },
    ...
    { key: 'fta', label: 'FTA', required: false, synonyms: ['fta', 'ft att', 'ft attempts'], help: 'Required unless FT% cell contains (makes/attempts)' },
    { key: 'ftPct', label: 'FT%', required: false, synonyms: ['ft%', 'ft pct', 'ftpct', 'ft_pct', 'ft'] },
    { key: 'threes', label: '3PM', required: true, synonyms: ['3pm', '3ptm', '3p', 'threes', '3pt', 'tpm', '3-pt'] },
    ...
    { key: 'statBasis', label: 'Stat basis (PER_GAME/TOTAL)', required: false, synonyms: ['basis', 'stat basis'] },
  ...
  YAHOO_MARKET: [
    ...
    POS,
    PID,
    { key: 'xrank', label: 'Yahoo XRank', required: false, synonyms: ['xrank', 'x rank', 'x-rank', 'o-rank', 'orank'] },
    { key: 'rank', label: 'Yahoo Rank', required: false, synonyms: ['rank', 'rk', 'current rank'] },
    { key: 'adp', label: 'Yahoo Last 7 Days ADP', required: false, synonyms: ['adp', 'l7 adp', 'last 7 days adp', 'adp7', 'adp (l7)', 'avg pick', 'last 7 days'] },
  ...
  AVAILABILITY: [
    ...
    { key: 'teamGames', label: 'Team games', required: false, synonyms: ['team games', 'teamgames', 'team gp'], help: 'Defaults to season length' },
    { key: 'missedLow', label: 'Missed — LOW recurrence', required: false, synonyms: ['missed low', 'low'] },
    { key: 'missedModerate', label: 'Missed — MODERATE', required: false, synonyms: ['missed moderate', 'moderate'] },
    { key: 'missedHigh', label: 'Missed — HIGH recurrence', required: false, synonyms: ['missed high', 'high'] },
    { key: 'missedUnclassified', label: 'Missed — unclassified', required: false, synonyms: ['missed unclassified', 'unclassified'] },
  ...
  CONTEXT: [
    ...
    { key: 'status', label: 'Current status', required: false, synonyms: ['status', 'current status', 'injury'] },
    { key: 'recoveryNote', label: 'Recovery note', required: false, synonyms: ['recovery', 'recovery note'] },
    { key: 'manualRiskDelta', label: 'Risk adjustment (−0.3…0.3)', required: false, synonyms: ['risk delta', 'risk adj', 'manual risk'] },
    { key: 'manualUpside', label: 'Manual upside (0–1)', required: false, synonyms: ['manual upside', 'upside'] },
    { key: 'providerUpside', label: 'Provider upside (0–1)', required: false, synonyms: ['provider upside'] },
    { key: 'roleTags', label: 'Role tags (; separated)', required: false, synonyms: ['role tags', 'tags', 'role'] },
    { key: 'previousSeasonMpg', label: 'Previous-season MPG', required: false, synonyms: ['prev mpg', 'previous mpg', 'last season mpg'] },
...
export function autoMapColumns(kind: ImportKind, headers: readonly string[]): Record<string, string | null> {
  ...
  for (const spec of specs) {
    const hit = headers.find((h) => !used.has(h) && (spec.synonyms.includes(canon(h)) || canon(h) === spec.key.toLowerCase()));
