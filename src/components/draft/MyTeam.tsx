  ...
  return (
    <Panel title={`My team (${ev.roster.length}/${ev.positions.activeSlots + league.roster.bench})`}>
      <table className="w-full whitespace-nowrap text-[11px]" data-testid="my-team">
        <thead className="text-left text-[10px] uppercase text-slate-500">
          <tr>
            ...
            <th>Pick</th>
            <th>Player</th>
            <th className="text-right" title="Neutral 9-cat (raw z, TO 1.0)">
              N9
            </th>
            <th className="text-right" title="Team fit at the time drafted">
              Fit
            </th>
          ...
          {slots.map((s, i) => {
            ...
            return (
              <tr key={i} className="border-t border-slate-100 dark:border-slate-800">
                ...
                <td className="pl-1 text-slate-600 dark:text-slate-300">
                  {r?.keyCategories.map((c) => CATEGORY_LABEL[c]).join(' ')}
                </td>
                <td>{r?.risk ?? ''}</td>
                <td className="text-right">
                  {r && (
                    <Button
                      size="xs"
                      variant="ghost"
                      title="Remove this pick (clock does not move; undoable)"
                      onClick={() => onRemove(r.seq)}
                    >
                      ✕
                    </Button>
                  )}
      ...
      <p className="mt-1 text-[10px] text-slate-500">
        Active slots filled {ev.positions.filledSlots}/{ev.positions.activeSlots} ·{' '}
        {ev.positions.feasible ? 'roster feasible' : 'ROSTER INFEASIBLE'} · required:{' '}
