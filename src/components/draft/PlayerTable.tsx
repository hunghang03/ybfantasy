...
export function filterAndSort(
  ...
  const posOk = (p: PlayerEvaluation) =>
    f.position === 'ALL' ||
    (f.position === 'G'
      ? p.positions.some((x) => x === 'PG' || x === 'SG')
      : f.position === 'F'
        ? p.positions.some((x) => x === 'SF' || x === 'PF')
...
export function PlayerTable({
  ...
  onAction: (
    id: string,
    action: 'MINE' | 'OTHER' | 'favorite' | 'avoid' | 'doNotDraft' | 'lockTarget',
  ) => void;
  ...
  const th = (key: SortKey, label: string, cls = '') => (
    <th
      className={cx(
        'cursor-pointer select-none px-1 py-1 font-semibold',
        cls,
        sort === key && 'text-blue-700 dark:text-blue-300',
      )}
      onClick={() => setSort(key)}
      scope="col"
    >
    ...
    >
      <div className="flex flex-wrap items-center gap-1.5 border-b border-slate-200 p-1.5 text-xs dark:border-slate-800">
        ...
        <Select
          ...
          onChange={(e) => set({ position: e.target.value as TableFilters['position'] })}
        ...
        <Select
          ...
          value={filters.label}
          onChange={(e) => set({ label: e.target.value as TableFilters['label'] })}
          className="py-0.5 text-xs"
        >
        ...
        <Select
          ...
          value={filters.risk}
          onChange={(e) => set({ risk: e.target.value as TableFilters['risk'] })}
          className="py-0.5 text-xs"
        >
        ...
        <span className="ml-auto text-slate-500">
          {rows.length} available · keys: ↑↓ select · M mine · D taken{catchUp ? ' (no advance)' : ''} · F ★ ·
          A avoid · L lock · U undo · Enter details
        </span>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <td className={cx('px-1 font-medium', pad)}>
                    ...
                    {p.flags.favorite && (
                      <span className="ml-1 text-amber-500" title="Favorite">
                        ★
                      </span>
                    )}
                    {p.flags.lockTarget && (
                      <span className="ml-1" title="Locked target">
                        🔒
                      </span>
                    )}
                    {p.flags.avoid && <span className="ml-1 text-[10px] text-slate-500">avoid</span>}
                    {p.availability.status !== 'HEALTHY' && (
                      <span className="ml-1 text-[10px] font-bold text-red-600">{p.availability.status}</span>
                    )}
                    {p.disagreement.some((d) => d.flagged) && (
                      <span className="ml-1 text-[10px] text-amber-600" title="Projection disagreement">
                        Δ
                      </span>
                    )}
                    {p.confidence === 'LOW' && (
                      <span className="ml-1 text-[10px] text-slate-400" title={p.warnings.join('; ')}>
                        low data
                      </span>
                    )}
                  </td>
                  ...
                  <td
                    className={cx('num px-1 text-right font-semibold', pad)}
                    title={`raw ${p.ddpRaw.toFixed(2)}`}
                  >
                    {p.ddpScore}
                  </td>
                  ...
                  <td
                    className={cx('whitespace-nowrap px-1 text-right', pad)}
                  ...
                  >
                    ...
                    <Button
                      ...
                      variant="ghost"
                      title="Favorite (F)"
                      aria-pressed={p.flags.favorite}
                      onClick={() => onAction(p.playerId, 'favorite')}
                    >
                      ★
                    </Button>
                    <Button
                      ...
                      variant="ghost"
                      title="Avoid (A)"
                      aria-pressed={p.flags.avoid}
                      onClick={() => onAction(p.playerId, 'avoid')}
                    >
                      ⊘
                    </Button>
                    <Button
                      ...
                      variant="ghost"
                      title="Do not draft"
                      aria-pressed={p.flags.doNotDraft}
                      onClick={() => onAction(p.playerId, 'doNotDraft')}
                    >
                      DND
                    </Button>
                    <Button
                      ...
                      variant="ghost"
                      title="Lock target (L)"
                      aria-pressed={p.flags.lockTarget}
                      onClick={() => onAction(p.playerId, 'lockTarget')}
                    >
                      🔒
                    </Button>
                  </td>
