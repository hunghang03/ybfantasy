...
import { DEFAULT_FILTERS, filterAndSort, PlayerTable, type SortKey, type TableFilters } from '@/components/draft/PlayerTable';
...
export default function DraftPage() {
  ...
  const lastLabel = useMemo(() => {
    ...
    const name = ctx.byId.get(lastEvent.playerId)?.player.name ?? ctx.unranked.find((u) => u.id === lastEvent.playerId)?.name ?? lastEvent.playerId;
  ...
  const act = useCallback(
    (id: string, action: 'MINE' | 'OTHER' | keyof PlayerFlags) => {
      ...
      if (action === 'MINE' || action === 'OTHER') {
        ...
        const snapshot = p ? { ddpRaw: p.ddpRaw, baseValue: p.value.basePlayerValue, teamFit: p.fit.teamFit, label: p.label } : undefined;
        const err = draftPick(id, action === 'MINE' ? 'ME' : 'OTHER', { advance: action === 'MINE' ? true : !catchUp, snapshot });
        if (err) notify(err);
        else if (action === 'MINE' && !ev.timing.onTheClock) notify(`Recorded as your pick at #${ev.draft.currentOverall}, which is not on your snake schedule. Use Resync if Yahoo differs.`);
        return;
      }
  ...
  if (!league || !draft)
    return (
      <div className="p-6 text-sm">
        No league selected. <Link className="text-blue-600 underline" href="/">Create or select a league</Link>.
      </div>
    );
  ...
  return (
    <div className="space-y-2 p-2">
      <div className="sticky top-[37px] z-20 space-y-2 bg-slate-50 pb-1 dark:bg-slate-950">
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-md border border-slate-200 bg-white px-2 py-1 text-xs dark:border-slate-800 dark:bg-slate-900" data-testid="draft-controls">
          <span>
            Pick <b className="num" data-testid="current-pick">{ev.draft.currentOverall}</b> · Round <b className="num">{ev.timing.currentRound}</b>
          </span>
          <span>
            Your next pick <b className="num" data-testid="next-pick">{ev.timing.p0 ?? '—'}</b>
            {ev.timing.onTheClock && <b className="ml-1 rounded bg-red-600 px-1 text-white">ON THE CLOCK</b>}
          </span>
          <span>
            then <b className="num" data-testid="following-pick">{ev.timing.p1 ?? '—'}</b> · {ev.gap.beforeNext} picks before your next
            {ev.timing.gapType && <> · {ev.timing.gapType.toLowerCase()} gap</>}
          </span>
          <Button size="sm" data-testid="undo" onClick={undo} disabled={draft.events.length === 0} title="Undo last action (U)">
            ↶ Undo
          </Button>
          {lastLabel && <span className="text-slate-500" data-testid="last-event">Last: {lastLabel}</span>}
          <span className="ml-auto flex items-center gap-1">
            ...
            <label className="ml-2 flex items-center gap-1" title="Taken marks players unavailable without advancing the pick">
              <input type="checkbox" data-testid="catch-up" checked={catchUp} onChange={(e) => setCatchUp(e.target.checked)} />
              Catch-up (no advance)
            </label>
        ...
        {st.unrecordedPicks > 0 && (
          <p className="rounded bg-amber-100 px-2 py-0.5 text-xs text-amber-900 dark:bg-amber-900 dark:text-amber-100" data-testid="unrecorded-banner">
            {st.unrecordedPicks} pick(s) not recorded locally. Turn on catch-up mode and mark the missing players as Taken.
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-2 xl:grid-cols-[1fr_27rem]">
        ...
        <div className="space-y-2">
          ...
          <MyTeam
            ...
            league={league}
            onRemove={(seq) => {
              if (!window.confirm('Remove this pick from your team? The pick clock does not move (undoable).')) return;
      ...
      {detail && (
        <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/30 p-2 md:items-center" onClick={() => setDetailId(null)} role="dialog" aria-modal="true" aria-label="Player details">
          <div className="max-h-[90vh] w-full max-w-6xl overflow-auto rounded-md bg-white p-3 shadow-xl dark:bg-slate-900" onClick={(e) => e.stopPropagation()}>
      ...
      {ev.warnings.some((w) => w.code === 'NON_FINITE_REPAIRED') && <Panel title="Engine">Non-finite values were repaired; see Review.</Panel>}
    </div>
  );
}
