...
function SetupForm({ league }: { league: LeagueProfile }) {
  ...
  const providers = useMemo(
    () => [...new Set(dataset.projections.map((p) => p.provider))].sort(),
    [dataset.projections],
  );

  const rounds = rosterSize(form.roster);
  const picks =
    validateSnake({ teams: form.teamCount, slot: form.draftPosition, rounds }).length === 0
      ? userPicks({ teams: form.teamCount, slot: form.draftPosition, rounds })
      : [];
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-3 p-3">
      <Panel
        title={`League setup — ${league.name}`}
        actions={
          <Button variant="primary" size="sm" data-testid="save-league" onClick={save}>
            Save
          </Button>
        }
      >
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <Field label="Number of teams">
            <Input
              ...
              onChange={(e) => set('teamCount', num(e.target.value))}
            />
          </Field>
          <Field label="Your draft position">
            <Input
              data-testid="draft-position"
              type="number"
              min={1}
              max={form.teamCount}
              value={form.draftPosition}
              onChange={(e) => set('draftPosition', num(e.target.value))}
            />
          </Field>
          ...
          <Field label="Acquisitions / week">
            <Input
              ...
              min={0}
              max={50}
              value={form.acquisitionsPerWeek}
              onChange={(e) => set('acquisitionsPerWeek', num(e.target.value))}
            />
          </Field>
          ...
          <Field label="Primary projection source" hint="Validation sources are compared, never averaged">
            <Input
              list="providers"
              value={form.primaryProjectionProvider}
              onChange={(e) => set('primaryProjectionProvider', e.target.value.trim())}
            />
          </Field>
        ...
        <h3 className="mt-4 mb-1 text-xs font-semibold uppercase text-slate-500">
          Roster (Yahoo default: PG SG G SF PF F C C UTIL UTIL + bench; no IL+)
        </h3>
        <div className="grid grid-cols-5 gap-2 md:grid-cols-10">
            ...
            <Field key={s} label={s}>
              <Input
                ...
                onChange={(e) =>
                  set('roster', {
                    ...form.roster,
                    active: { ...form.roster.active, [s]: num(e.target.value) },
                  })
                }
              />
            </Field>
          ...
          <Field label="Bench">
            <Input
              ...
              min={0}
              max={10}
              value={form.roster.bench}
              onChange={(e) => set('roster', { ...form.roster, bench: num(e.target.value) })}
            />
          </Field>
          <Field label="IL">
            <Input
              ...
              min={0}
              max={6}
              value={form.roster.il}
              onChange={(e) => set('roster', { ...form.roster, il: num(e.target.value) })}
            />
          </Field>
        </div>
        ...
        {inProgress &&
          (form.draftPosition !== league.draftPosition || form.teamCount !== league.teamCount) && (
            <p className="mt-2 text-xs text-amber-700">
              A draft is in progress. Changing teams/position regenerates your pick schedule; recorded picks
              are kept (off-schedule picks are flagged).
            </p>
          )}
      </Panel>
      ...
      <Panel title="Your snake picks (generated)">
        <div className="flex flex-wrap gap-1" data-testid="user-picks">
          {picks.map((p, i) => (
            <span
              key={p}
              className="num rounded bg-slate-100 px-1.5 py-0.5 text-xs dark:bg-slate-800"
              title={`Round ${i + 1}`}
            >
              {p}
            </span>
      ...
      <Panel title="Draft state">
        <p className="text-sm">
          Current overall pick <b className="num">{st.currentOverall}</b> · {st.myPicks.length} of your picks
          recorded · {draft?.events.length ?? 0} events.
        </p>
        <Button
          className="mt-2"
          variant="danger"
          ...
          disabled={!inProgress}
          onClick={() => {
            if (window.confirm('Reset this league’s draft (all picks)? Flags and punt settings are kept.'))
              resetDraft();
          }}
        >
          Reset draft
